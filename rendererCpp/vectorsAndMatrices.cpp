...
const static float epsilon = 0.000001f; //używany do sprawdzania, czy normalizowany wektor nie jest za mały
...
//_mm_shuffle_ps tworzy wektor _m128, w który wkłada 2 elementy z pierwszego argumentu i 2 elementy z drugiego argumentu
//więc jeśli chcemy przemieszać kolejność jednego wektora, to podajemy go dwa razy do funkcji
//_MM_SHUFFLE(3, 0, 2, 1) określa jak zostaną ustawione elementy
//w tym wypadku:
//-wzięte z drugiego wektora - na ostatnim miejscu będzie element czwarty i ostatni (3), na trzecim element pierwszy (0),
//-wzięte z pierwszego wektora - na drugim miejscu element trzeci (2), a na pierwszym element drugi (1)
//pamiętajmy, że kolejność elementów w wypadku tych funkcji jest odwrócona - stąr pierwszy argument _MM_SHUFFLE, czyli 3, odnosi się do ostatniego elementu wektora
...
//todo, czy w pasce powinno być F = 15?
...
void vector4::scaleToOne()
{
	...
	a = max(0.0f, min(1.0f, a)); //alfy nie skaluję z resztą koloru - jeśli jest większa od 1 ustawiam na 1
}
...
string float4x4::toString()
{
	...
	for (i = 0; i < 4; ++i) {
		for (j = 0; j < 4; ++j) {
			matrix += to_string((int)(*this)[i][j]) + " "; //liczby całkowite zajmują mniej miejsca - łatwiej czytać output
